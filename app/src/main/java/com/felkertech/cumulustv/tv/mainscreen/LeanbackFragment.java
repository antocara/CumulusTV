...
import com.felkertech.cumulustv.data.model.SuggestedChannels;
...
import com.felkertech.cumulustv.tv.fragments.VideoDetailsFragment;
...
public class LeanbackFragment extends BrowseFragment
  ...
  private final GoogleDriveBroadcastReceiver broadcastReceiver =
      new GoogleDriveBroadcastReceiver() {
        @Override public void onDownloadCompleted() {
          refreshUI();
          Toast.makeText(getActivity(), R.string.toast_sync_data_downloaded, Toast.LENGTH_SHORT)
              .show();
        }

        @Override public void onUploadCompleted() {
          refreshUI(); // Probably need to reload anyway
          Toast.makeText(getActivity(), R.string.toast_sync_data_uploaded, Toast.LENGTH_SHORT)
              .show();
        }

        @Override public void onNetworkActionCompleted() {
          if (DEBUG) {
  ...
  private void loadRows() {
    ...
    ChannelDatabase cd;
    try {
      cd = ChannelDatabase.getInstance(getActivity());
    ...
    HeaderItem suggestedChannelsHeader =
        new HeaderItem(ORDER_SUGGESTED_CHANNELS, getString(R.string.suggested_channels));
    ...
    // Third row is Drive
    HeaderItem driveHeader =
        new HeaderItem(ORDER_GOOGLE_DRIVE, getString(R.string.google_drive_sync));
    ...
    gridRowAdapter2.add(
        new Option(getResources().getDrawable(R.drawable.ic_cog), getString(R.string.settings)));
  ...
  @Override public void onConnected(Bundle bundle) {
    ...
    sm.setGoogleDriveSyncable(CloudStorageProvider.getInstance().getClient(),
        new DriveSettingsManager.GoogleDriveListener() {
          @Override public void onActionFinished(boolean cloudToLocal) {
            ...
            CumulusJobService.requestImmediateSync1(getActivity(), info,
    ...
    if (sm.getString(R.string.sm_google_drive_id).isEmpty()) {
      //We need a new file
      ActivityUtils.createDriveData(getActivity(), CloudStorageProvider.getInstance().getClient(),
          driveContentsCallback);
    } else {
      //Great, user already has sync enabled, let's resync
      ActivityUtils.readDriveData(getActivity(), CloudStorageProvider.getInstance().getClient());
        ...
        @Override public void onResult(DriveApi.DriveContentsResult result) {
          MetadataChangeSet metadataChangeSet =
              new MetadataChangeSet.Builder().setTitle("cumulustv_channels.json")
                  .setDescription(
                      "JSON list of channels that can be imported using CumulusTV to view live streams")
    ...
    if (connectionResult.hasResolution()) {
      try {
        connectionResult.startResolutionForResult(getActivity(),
            ActivityUtils.RESOLVE_CONNECTION_REQUEST_CODE);
      } catch (IntentSender.SendIntentException e) {
    ...
    @Override public void onItemClicked(Presenter.ViewHolder itemViewHolder, Object item,
      ...
      if (item instanceof JsonChannel) {
        ...
        if (title.equals(getString(R.string.manage_livechannels))) {
          ...
          ActivityUtils.openSuggestedChannels(getActivity(),
          ...
          CloudStorageProvider.getInstance().connect(getActivity());
        } else if (title.equals(getString(R.string.settings_switch_google_drive))) {
          CloudStorageProvider.getInstance().pickDriveFile(getActivity());
          ...
          ActivityUtils.deleteChannelData(getActivity(),
              CloudStorageProvider.getInstance().getClient());
